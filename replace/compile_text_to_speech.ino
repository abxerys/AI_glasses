...
// ===== 版本: v2.4-SPIIMU - ICM42688 改? SPI，避? I2S 干扰；WAV chunked 播放保持 =====
...
#define FB_COUNT      2
volatile int g_target_fps = 2; // 新增：0=不限，>0 ?按?FPS限速?送

// 【新增】????性能?控
volatile unsigned long frame_captured_count = 0;  // 采集???
volatile unsigned long frame_sent_count = 0;      // ?送???
volatile unsigned long frame_dropped_count = 0;   // ?????
volatile unsigned long last_stats_time = 0;       // 上次????
volatile unsigned long ws_send_fail_count = 0;    // WebSocket?送失???
...
volatile bool snapshot_in_progress = false; // 抓拍期??停??采集
...
bool init_camera() {
  ...
  if (s) {
    ...
    s->set_hmirror(s, 1);  // ★ 新增：水平?像，与人眼左右一致（1=?，0=?）
    s->set_vflip(s, 0);    // ★ 新增：垂直翻?；若??“倒?”，改? 1
...
void taskCamSend(void*) {
  ...
  for(;;){
    ...
    if (xQueueReceive(qFrames, &fb, pdMS_TO_TICKS(100)) == pdPASS) {
      if (fb && cam_ws_ready) {
        // ?送?流：若?置了目?FPS，?按周期?，??多余?由 qFrames 机制承?
        if (g_target_fps > 0) {
...
void init_i2s_in(){
  ...
  // 3. 強制指定 I2S_SLOT_MODE_LEFT (只讀取左聲道，這對單聲道 PDM 麥克?很重要)
...
void warmup_mic() {
  float dc = 0;
  for (int i = 0; i < 1600; i++) {  // 100ms @ 16kHz
    int v = i2sIn.read();
    if (v != -1) dc += 0.05 * (v - dc);
    delayMicroseconds(50);
  }
  Serial.printf("[MIC] warmup done, DC=%.0f\n", dc);
}
// ====================================================================
// Setup / Loop
// ====================================================================
void setup() {
  ...
  qFrames = xQueueCreate(3, sizeof(fb_ptr_t));  // 增加到3???，?少??
  qAudio  = xQueueCreate(AUDIO_QUEUE_DEPTH, sizeof(AudioChunk));
  qTTS    = xQueueCreate(TTS_QUEUE_DEPTH, sizeof(TTSChunk));

  xTaskCreatePinnedToCore(taskCamCapture, "cam_cap", 10240, NULL, 2, NULL, 0);
  xTaskCreatePinnedToCore(taskCamSend,    "cam_snd",  8192, NULL, 2, NULL, 0);
  xTaskCreatePinnedToCore(taskMicCapture, "mic_cap",   4096, NULL, 4, NULL, 1);
  xTaskCreatePinnedToCore(taskMicUpload,  "mic_upl",   4096, NULL, 3, NULL, 1);
