...
class NavigationMaster:
    def __init__(self,
        ...
        self.last_display_ts = 0.0          # 文字顯示計時器
        self.min_display_interval = 0.5     # 文字更新頻率 (0.5秒)
    ...
    # 處理網頁文字頻率
    def _display(self, now: float, text: str) -> str:
        if not text:
            return ""
        if now - self.last_display_ts >= self.min_display_interval:
    ...
    def process_frame(self, bgr: np.ndarray) -> OrchestratorResult:
        ...
        if self.state in (BLINDPATH_NAV, SEEKING_CROSSWALK, SEEKING_NEXT_BLINDPATH, RECOVERY):
            ...
            try:
                ...
                return OrchestratorResult(ann_err, self._say(now, ""), display_text=self._display(now, say), state=self.state, extras={"error": str(e)})
            ...
            return OrchestratorResult(ann, self._say(now, say), display_text=self._display(now, say), state=self.state, extras={"source": "blind", "cross_stage": cross_stage, "blind_state": blind_state})
        ...
        if self.state == WAIT_TRAFFIC_LIGHT:
            ...
            return OrchestratorResult(ann, self._say(now, say), display_text=self._display(now, say), state = self.state, extras={"traffic_light": major})
        ...
        if self.state == CROSSING:
            ...
            return OrchestratorResult(ann, self._say(now, say), display_text=self._display(now, say), state = self.state, extras={"source": "cross", "end_cnt": self.cnt_cross_end})
